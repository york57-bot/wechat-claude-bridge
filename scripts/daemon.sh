...
# Supports: macOS (launchd) / Linux (systemd + nohup fallback) / Windows (Git Bash)
# =============================================================================
...
main() {
  ...
  case "$OS_TYPE" in
    ...
    Linux)
      ...
      ;;
    MINGW*|MSYS*|CYGWIN*)
      # Windows Git Bash / MSYS2 — direct mode (same as Linux nohup fallback)
      WIN_NODE="$(command -v node || echo 'node')"
      win_pid_file() { echo "${DATA_DIR}/${SERVICE_NAME}.pid"; }

      win_start() {
        local pid_file="$(win_pid_file)"
        if [ -f "$pid_file" ]; then
          local old_pid=$(cat "$pid_file" 2>/dev/null)
          if [ -n "$old_pid" ] && kill -0 "$old_pid" 2>/dev/null; then
            echo "Already running (PID: $old_pid)"
        ...
        mkdir -p "$DATA_DIR/logs"
        echo "Starting wechat-claude-code daemon (Windows direct mode)..."
        nohup "$WIN_NODE" "${PROJECT_DIR}/dist/main.js" start \
          >> "$DATA_DIR/logs/stdout.log" \
          2>> "$DATA_DIR/logs/stderr.log" &
        local pid=$!
        echo "$pid" > "$pid_file"
      ...
      win_restart() { win_stop; sleep 1; win_start; }
    ...
    *)
      ...
      echo "Supported platforms: macOS (Darwin), Linux, Windows (Git Bash / MSYS2)"
      exit 1
      ;;
  esac
}
